    ...
    db.run(
      ...
      (err) => {
        if (err) {
          ...
          db.run(
            `INSERT INTO data(id, timestamp, crash_point, pre_crash_point, predict_crash_point, risk_at) VALUES(1,'-',0,0,0,0)`,
            (err) => {
              if (err) {
                // Table already created
                console.log(err.message);
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          ...
          db.run(
            `INSERT INTO threads(id, thread, rounds, value, status) VALUES(1,'thread_1',0, 100, True), (2,'thread_2',0, 100, True), (3,'thread_3',0, 100, True)`,
            (err) => {
              if (err) {
                // Table already created
                console.log(err.message);
